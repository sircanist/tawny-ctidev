...
(defindividual iBusinessImpactRelationship)
(refine iBusinessImpactRelationship
    :type Relationship
...
(di iImpactAssessment
    ...
    :fact
    (is created-by iBobMayerIdentity)
    (is has-relationship iBusinessImpactRelationship)
    ;(is business-impact Ind-BusinessImpact-None)
    )

(defindividual iIncidentStatusRelationship)
    (refine iIncidentStatusRelationship
    :type Relationship
    ;; not supported by weakener at the moment (ELDescriptor)
    ;; (has-self is-a-incident-status)
    :fact
...
(di iIncidentGrouping
    ...
    (is grouping-ref iIncidentStatusRelationship)
    (is grouping-ref iBusinessImpactRelationship)
